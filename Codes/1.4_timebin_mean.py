...
OUT_PREFIX = r"C:\Users\22457\Desktop\test\O_Mean_"

# TimeBin window widths: explicitly listed or generated via range
TIMEBIN_LIST = list(range(10, 19, 10))
...
# Plot title prefix
TITLE_PREFIX = "O_Mean_"
# ========================================================================

def calc_mean(y):
    """Return mean for a 1-D array-like; if n == 0 return np.nan."""
    arr = np.asarray(y)
    if arr.size == 0:
        return np.nan
...
def run_one_timebin(tb_width, data, age_nodes):
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        df_out.to_excel(writer, index=False, sheet_name="TimeBin_Mean")
    print(f"Saved Excel: {excel_path}")

    # ----------- Plot and save PNG -----------
    df_plot = df_out.sort_values("Age_node", ascending=False)
    plt.figure(figsize=(30, 6))
    plt.plot(df_plot["Age_node"], df_plot["Mean"],
